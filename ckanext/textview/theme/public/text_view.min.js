...
jQuery.ajax(resource_url,{type:'GET',headers:{"Range": "bytes=0-2000000"},contentType:p.contentType,dataType:p.dataType,success:function(data,textStatus,jqXHR){data=p.dataConverter?p.dataConverter(data):data;var highlighted;if(p.language){highlighted=hljs.highlight(p.language,data,true).value;}else{highlighted='<pre>'+data+'</pre>';}
self.el[0].innerHTML=highlighted;},error:function(jqXHR,textStatus,errorThrown){if(textStatus === 'parseerror' && jqXHR.responseText){var highlighted;if (p.language) {highlighted = hljs.highlight(p.language, jqXHR.responseText, true).value;} else {highlighted = '<pre>' + jqXHR.responseText + '</pre>';}self.el[0].innerHTML = highlighted;} else if(textStatus=='error'&&jqXHR.responseText){self.el.html(jqXHR.responseText);}else{self.el.html(self._('An error occured during AJAX request. Could not load view.'));}}});}};});
